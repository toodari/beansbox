...
public class AdjustServiceImpl implements AdjustService{
    ...
    public Long register(AdjustOrdersDTO adjustOrdersDTO, List<Long> pnum, List<Long> pquantity) {
        Orders orders = modelMapper.map(adjustOrdersDTO, Orders.class);
        ordersRepository.save(orders);
        Long onum = orders.getOnum();

        for (int i = 0; i < pnum.size(); i++) {
            AdjustOrderDetailDTO adjustOrderDetailDTO = AdjustOrderDetailDTO.builder()
            ...
            OrderDetail orderDetail = modelMapper.map(adjustOrderDetailDTO, OrderDetail.class);
            orderDetailRepository.save(orderDetail);

            List<Object[]> result = productRepository.getProductWithImage(pnum.get(i));
            Product product = (Product) result.get(0)[0];
            Long finalQuantity = pquantity.get(i);
